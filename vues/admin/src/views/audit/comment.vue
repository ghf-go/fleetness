      ...
      <el-table-column label="操作" fixed="right">
        <template #default="scope">
          <el-button
            ...
            type="success"
            size="small"
            @click.prevent="audit(scope.row, 'success')"
          >
            通过
          </el-button>
          <el-button
          ...
          <el-button
            ...
            type="waring"
            size="small"
            @click.prevent="audit(scope.row, 'self')"
          >
            仅作者可见
          </el-button>
        </template>
...
export default {
  ...
  methods: {
    async audit(r, act) {
      const data = await this.$api("/comment/audit", {
        id: r.id,
        act: act,
      });
      if (data.code == 200) {
    ...
    async loadData(page) {
      ...
      const data = await this.$api("/comment/wait_audit", this.queryData);
      if (data.code != 200) {
        this.$message.error(data.msg);
      } else {
