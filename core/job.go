...
func (ja jobAfter) start(ctx *GContent) {
	defer func() {
		if e := recover(); e != nil {
			AppDebug("%s任务错误 %v", ja.name, e)
			ja.start(ctx)
		}
	}()
	for {
...
func (ja jobAlway) start(ctx *GContent) {
	defer func() {
		if e := recover(); e != nil {
			AppDebug("%s任务错误 %v", ja.name, e)
			ja.start(ctx)
...
func (ja jobCron) start(ctx *GContent, month time.Month, d, h, m int, wd time.Weekday) {
	defer func() {
		if e := recover(); e != nil {
			AppDebug("%s任务错误 %v", ja.name, e)
		}
...
func (ja jobCron) check(val string, checkVal int) bool {
	if strings.HasPrefix(val, "*/") {
		if v, e := strconv.Atoi(val[2:]); e == nil {
			if checkVal%v > 0 {
				AppDebug("%s : %d -> %d -> %d", val, checkVal, v, checkVal%v)
				return false
			}
		} else {
			AppDebug("%s : %d -> 解析数据失败", val, checkVal)
			return false
		}
		...
		if _, ok := um[checkVal]; !ok {
			AppDebug("%s : %d -> 数据内容 %v", val, checkVal, um)
			return false
		}
	} else if strings.Index(val, "-") > 0 {
		ss := strings.Split(val, "-")
		...
		if checkVal < min || checkVal > max {
			AppDebug("%s : %d -> 时间范围 %d - %d ", val, checkVal, min, max)
			return false
		}
	} else if val != "*" {
		if v, _ := strconv.Atoi(val); v != checkVal {
			AppDebug("%s : %d -> 数据错误 %d  ", val, checkVal, v)
			return false
		}
	}
	return true
